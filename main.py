...
from log import fmt_timestamp, log_message, log, write_log
import sys
import subprocess
...
def main():
    global SAVE_CHAT_LOG
    # make sure that ollama is installed and running
    try:
        check_version(supress=True)
    except:
    ...
    log("INFO:")
    log("generated by ollama-client (https://github.com/pancake423/ollama-client)")
    log(f"model = {model}")
    log(f"timestamp = {fmt_timestamp(sep_chars=("-", " ", ":"))}\n")
    log("BEGIN CHAT LOG:")
